...
test('Zotero lifecycle registers once, does not launch eagerly, and unregisters returned IDs', async () => {
  ...
  assert.deepEqual(Array.from(h.registrations.preferences.scripts), ['content/browser.js', 'content/preferences.js']);
  let enabled;
...
for (const mode of ['chrome', 'edge']) test(`native browser adapter honors ${mode} and launches Unicode arguments without a shell`, async () => {
  const h = runtime();
  h.context.URL = URL;
  h.context.Zotero.isWin = true;
  h.context.Zotero.Prefs.get = key => key === 'researchAssistant.browserMode' ? mode : '';
  h.context.Services.env = { get: key => key === 'LOCALAPPDATA' ? 'C:\\Users\\测试\\AppData\\Local' : '' };
  h.context.PathUtils = { join: path.win32.join };
  const executable = 'C:\\Users\\测试\\AppData\\Local\\' + (mode === 'chrome' ? 'Google\\Chrome\\Application\\chrome.exe' : 'Microsoft\\Edge\\Application\\msedge.exe');
  h.context.IOUtils = { exists: async value => value === executable };
  const calls = [];
  let launchedPath;
  const process = { init: file => { launchedPath = file.path; }, runwAsync: (args, length, observer, weak) => {
    calls.push({ executable: launchedPath, args: Array.from(args) });
  ...
  h.context.Cc = {
    '@mozilla.org/file/local;1': { createInstance: () => ({
      ...
      exists() { return this.path === executable; }, isFile: () => true, isExecutable: () => true,
    }) },
  ...
  assert.deepEqual(calls, [{ executable, args: ['--disable-backgrounding-occluded-windows', 'https://gemini.google.com/app/saved#zra-connect=1'] }]);
  await h.context.shutdown({}, 4);
