...
// BEGIN SHARED TIMELINE (generated by scripts/build_userscript.cjs)
...
// BEGIN SHARED UPDATES (generated by scripts/build_userscript.cjs)
...
(function () {
  ...
  // Navigation is independent of Connector's task/lock/network lifecycle.
  // Only public, already-loaded user turns are indexed; never read editor drafts.
  class WebTimeline {
    constructor(config) {
      this.config = config;
      this.api = globalThis.ZoteroResearchTimeline;
      this.starKey = 'zra-timeline-stars-v1';
      this.preferenceKey = 'zra-timeline-visible-' + config.name;
      this.enabled = GM_getValue(this.preferenceKey, true) !== false;
      this.key = '';
      this.lastNodes = new Map();
      this.oldRouteNodes = new Map();
      this.host = document.createElement('div');
      this.host.setAttribute('data-zra-web-timeline', '');
      document.documentElement.appendChild(this.host);
      this.rail = this.api.mount(this.host, {
        web: true, enabled: this.enabled,
        onToggle: value => this.saveEnabled(value),
        onStar: (id, value) => this.saveStar(id, value),
      });
      this.schedule = () => {
        if (this.stopped || this.timer || typeof document === 'undefined' || !document || document.hidden) { return; }
        this.timer = setTimeout(() => { this.timer = null; this.refresh(); }, 200);
      };
      this.observer = new MutationObserver(records => {
        if (records.some(record => !this.host.contains(record.target))) { this.schedule(); }
      });
      this.onRoute = () => this.schedule();
      this.onVisibility = () => { if (!document.hidden) { this.schedule(); } };
      this.onHide = () => this.stop();
      this.onShow = () => this.start();
      window.addEventListener('pagehide', this.onHide);
      window.addEventListener('pageshow', this.onShow);
      GM_registerMenuCommand('显示／收起对话时间轴', () => {
        this.saveEnabled(!this.enabled); this.rail.setEnabled(this.enabled);
      });
      // Cross-tab bookmark updates do not touch connection locks.
      GM_addValueChangeListener(this.starKey, () => this.schedule());
      this.start();
    }
    ...
    start() {
      ...
      // SPA pushState does not fire popstate. Check only the URL, not the page,
      // on this slow tick; actual messages are driven by mutations.
      this.routeTimer = setInterval(() => {
        if (!document.hidden && (this.key !== this.conversationKey() || !this.host.isConnected)) { this.schedule(); }
      }, 1000);
      this.refresh();
    }
    ...
    stop() {
      this.stopped = true;
      this.observer.disconnect(); clearInterval(this.routeTimer); clearTimeout(this.timer); this.timer = null;
      window.removeEventListener('popstate', this.onRoute);
      document.removeEventListener('visibilitychange', this.onVisibility);
    }

    conversationKey() { return location.origin + location.pathname.replace(/\/$/, ''); }

    readBookmarks() {
      try {
        const value = GM_getValue(this.starKey, []);
        const records = typeof value === 'string' ? JSON.parse(value) : value;
        return Array.isArray(records) ? records.filter(record => typeof record?.key === 'string' && Array.isArray(record.ids)) : [];
      } catch (_) { return []; }
    }

    saveEnabled(value) {
      this.enabled = value;
      try { GM_setValue(this.preferenceKey, value); } catch (_) { notify('时间轴显示偏好未能保存，下次打开可能恢复默认。'); }
    }

    saveStar(id, value) {
      // A route can change between pointer-down and long-press completion.
      if (this.key !== this.conversationKey()) { this.schedule(); return; }
      const records = this.readBookmarks();
      const ids = new Set(records.find(record => record.key === this.key)?.ids || []);
      if (value) { ids.add(id); } else { ids.delete(id); }
      const others = records.filter(record => record.key !== this.key);
      const next = ids.size ? [{ key: this.key, ids: [...ids] }, ...others] : others;
      try { GM_setValue(this.starKey, next); }
      catch (_) { notify('星标未能保存到本机，刷新后可能丢失。'); }
    }

    collectUsers() {
      const selectors = {
        ChatGPT: '[data-turn="user"], [data-message-author-role="user"]',
        Gemini: 'user-query, user-query-content, .user-query-bubble-with-background',
        DeepSeek: '[data-um-id], ._4f9bf79, .ds-message[data-role="user"]',
        Kimi: '.chat-content-item-user, .chat-content-item.user, .chat-content-item[data-role="user"], .segment-user',
        Claude: '[data-testid="user-message"], [data-user-message-bubble="true"], .font-user-message',
        AIStudio: 'ms-chat-turn',
      };
      const candidates = [...document.querySelectorAll(selectors[this.config.name])].filter(node => {
        if (node.closest('form, [contenteditable="true"], [hidden], [aria-hidden="true"], nav, aside')) { return false; }
        if (this.config.name === 'AIStudio') {
          return node.matches('[data-turn-role="user" i], [data-role="user" i]')
            || Boolean(node.querySelector('.user-prompt-container, .chat-turn-container.user, [data-role="user" i], [data-turn-role="user" i]'));
        }
        return true;
      });
      // Prefer the outer turn over nested body/bubble wrappers.
      return candidates.filter(node => !candidates.some(parent => parent !== node && parent.contains(node)));
    }

    userText(node) {
      const body = node.querySelector('.query-text, [data-testid="user-message"], .user-prompt-container, .segment-user') || node;
      // textContent does not force layout on every streamed token. Remove UI
      // labels from a clone, never from the original conversation.
      const clone = body.cloneNode(true);
      clone.querySelectorAll('button, [role="button"], script, style, [aria-hidden="true"]').forEach(child => child.remove());
    ...
    refresh() {
      ...
      if (key !== this.key) {
        this.oldRouteNodes = this.lastNodes; this.lastNodes = new Map();
        this.key = key; this.rail.update([]);
      }
      const stars = new Set(this.readBookmarks().find(record => record.key === key)?.ids || []);
      const occurrences = new Map();
      const nextNodes = new Map();
      const entries = [];
      for (const node of this.collectUsers()) {
        const text = this.userText(node);
        const identity = node.getAttribute('data-turn-id') || node.getAttribute('data-message-id')
          || node.getAttribute('data-um-id') || node.id;
        const stamp = identity + '\n' + text;
        if (this.oldRouteNodes.has(node) && this.oldRouteNodes.get(node) === stamp) { continue; }
        nextNodes.set(node, stamp);
        const base = identity ? 'id:' + identity : 'text:' + this.api.fingerprint(text);
        const ordinal = occurrences.get(base) || 0; occurrences.set(base, ordinal + 1);
        const id = base + ':' + ordinal;
        entries.push({ id, text, target: node, starred: stars.has(id) });
      }
      this.lastNodes = nextNodes;
      this.rail.update(entries, entries.length ? this.api.scrollParent(entries[0].target) : undefined);
    }
  }
  ...
  class Connector {
    constructor(config) {
      ...
      this.runtime = {
        id: TAB_ID, startedAt: new Date().toISOString(), sourceRevision: 'conversation-timeline-1',
    ...
    initDom() {
      ...
      // Navigation failure must never interrupt the message relay.
      try { this.timeline = new WebTimeline(this.config); }
      catch (error) { console.warn('[Zotero timeline]', error); }
      createBadge();
      try { this.initUpdates(); } catch (error) { console.warn('[Zotero update check]', error); }
    ...
    initUpdates() {
      const api = globalThis.ZoteroResearchUpdates;
      if (!api?.validVersion(GM_info.script.version)) { return; }
      const checker = api.createChecker({
        kind: 'userscript', currentVersion: GM_info.script.version,
        readState: () => GM_getValue('zra-update-state-v1', {}),
        writeState: value => GM_setValue('zra-update-state-v1', value),
        fetchCatalog: url => new Promise((resolve, reject) => {
          let settled = false;
          const finish = (error, value) => {
            if (settled) { return; } settled = true; clearTimeout(deadline);
            if (error) { reject(error); } else { resolve(value); }
          };
          // A hard deadline also covers extension requests that omit ontimeout.
          const deadline = setTimeout(() => finish(new Error('timeout')), 9000);
          try {
            GM_xmlhttpRequest({
              method: 'GET', url, timeout: 8000, anonymous: true,
              onload: response => {
                try {
              ...
              onerror: () => finish(new Error('offline')), ontimeout: () => finish(new Error('timeout')),
            });
          } catch (error) { finish(error); }
        }),
      });
      const check = async (force = false) => {
        ...
        if (release.status === 'available' && release.notify) {
          let host = document.querySelector('[data-zra-update-notice]');
          if (!host) {
            host = document.createElement('div'); host.setAttribute('data-zra-update-notice', '');
            document.documentElement.appendChild(host);
          }
          api.showNotice(host, release, { kind: 'userscript', floating: true });
          checker.markNotified(release.version);
        } else if (force) {
          notify(release.status === 'unavailable' ? '暂时无法检查脚本更新，请检查 GitHub 网络连接后重试。' : '网页连接脚本已是最新版本。');
        }
      };
      GM_registerMenuCommand('检查脚本更新', () => { void check(true); });
      document.addEventListener('visibilitychange', () => { void check(); });
      setInterval(() => { void check(); }, 3600000);
      void check();
    }
